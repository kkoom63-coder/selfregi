...
const { PRICE, ORDER_RE, json, apiKey, tossPost, secret, signPass, modeAllowed } = require('./_lib');

const DONE = ['PAY_COMPLETE', 'SETTLEMENT_COMPLETE'];
...
module.exports = async (req, res) => {
  ...
  const orderNo = req.body && req.body.orderNo;
  if (!ORDER_RE.test(orderNo || '')) return json(res, 400, { ok: false, msg: '주문번호가 올바르지 않습니다.' });

  try {
    let st = await tossPost('/api/v2/status', { apiKey: key, orderNo });
    if (!st || st.code !== 0) return json(res, 404, { ok: false, msg: '결제 내역을 찾지 못했습니다.' });
    if (st.amount !== PRICE) return json(res, 409, { ok: false, msg: '결제 금액이 일치하지 않습니다.' });
    if (!modeAllowed(st.mode)) return json(res, 403, { ok: false, msg: '테스트 결제는 이용권이 발급되지 않습니다.' });
    ...
    if (st.payStatus === 'PAY_APPROVED') {
      ...
      if (!ex || ex.code !== 0) {
        ...
        if (!st || st.code !== 0 || !DONE.includes(st.payStatus)) {
          ...
          return json(res, 402, { ok: false, msg: (ex && ex.msg) || '결제 승인에 실패했습니다.' });
        }
      }
      ...
      const m = st.payStatus === 'PAY_CANCEL' ? '결제가 취소되었습니다.'
        ...
        : '결제가 아직 완료되지 않았습니다.';
      return json(res, 409, { ok: false, msg: m, payStatus: st.payStatus });
    }

    const pass = signPass(orderNo, Date.now());
    return json(res, 200, { ok: true, orderNo, token: pass.token, exp: pass.exp, amount: PRICE });
  } catch (e) {
    console.error('confirm error', orderNo, e && e.message);
    return json(res, 502, { ok: false, msg: '결제 확인 중 오류가 발생했습니다. 이 화면을 새로고침해 주세요.' });
  }
};
