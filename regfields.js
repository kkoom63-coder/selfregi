...
(function (root) {
  ...
  function extract(input, opt) {
    ...
    /*  [집합건물] 서울특별시 중구 ○○동 100-1 제3층 제301호
    ...
    for (var i = 0; i < S.length; i++) {
      ...
      if (!mh) continue;
      var body = mh[2];
      /* 머리글 뒤에 표 제목이 붙어 읽히는 경우가 있다(실측: '…제301호1.소유지분현황').
         식별자까지만 자른다. */
  ...
  /* 목록은 현행 17개다. 행정구역 개편으로 새 이름이 나오면 목록에 없다
     (픽스처 실측: '전남광주통합특별시'). 목록에서 못 찾으면 어미로 잡는다.
     '도'는 도로명 첫 글자를 삼키지 않도록 뒤 글자를 막아 둔다. */
  var RE_SIDO_ALT = /^[가-힣]{2,8}?(?:특별자치시|특별자치도|특별시|광역시)|^[가-힣]{2,3}도(?![로면리길])/;
  ...
  function respaceTail(t) {
    ...
    return String(t || '')
      .replace(/([가-힣])(제\d)/g, '$1 $2')          // '…건축물제1동' → '…건축물 제1동'
  ...
  function respaceAddr(s) {
    ...
    // 1단계 — 시·도
    m = t.match(RE_SIDO_HEAD) || t.match(RE_SIDO_ALT);
    if (!m) {
    ...
    if (t) {
      /* '상무민주로32번길'은 한 도로명이라 붙이고, '효령로 79길'은 띄운다.
         기존 회귀(seocho)와 텍스트 레이어 경로가 모두 이 표기를 쓴다. */
  ...
  /* '이 하 여 백'이 OCR 에서 '이' 한 글자만 남는 실물이 있다(2026-08-23 신당 스캔).
     안 걸러내면 주소 끝에 '이'가 붙고, 그 탓에 지점 괄호가 문장 끝이 아니게 되어
     지점 분리까지 같이 실패한다. 을구 본문에 단독 '이' 줄은 존재하지 않는다. */
  var EUL_STOP = /^(관할\s*등기소|열람일시|출력일시|이?하\s*여\s*백|이$|여백$|열람용|제출용|등기사항전부증명서|\[집합건물\]|\[토지\]|\[건물\]|\*|\d+\/\d+$|주요등기사항요약|\[?주의사항)/;
  ...
  /* 요약표 「1. 소유지분현황(갑구)」이 가장 깨끗하다.
       홍길동(소유자)단독소유800101-*******서울특별시○○구○○로100,301호(○○동)
       고다영(공유자)2분의1...
     못 찾으면 갑구 본문 「소유자 홍길동 800101-***…」로 되돌아간다.
  ...
  function extractOwners(S, K, out) {
    ...
    /* 서식 라벨이 이름 앞에 붙어 읽힌다('등기명의인홍길동'). 참고사항의
